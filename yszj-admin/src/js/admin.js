...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('yszj-upms-systemid', systemid);
		$.cookie('yszj-upms-systemname', systemname);
		$.cookie('yszj-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('yszj-upms-systemid') || 1;
	var systemname = $.cookie('yszj-upms-systemname') || 'yszj-upms-server';
	var systemtitle = $.cookie('yszj-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
